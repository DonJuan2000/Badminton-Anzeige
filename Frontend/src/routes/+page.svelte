...
<script lang="ts">
    ...
    onMount(() => {
        const spielstand_SSE = new EventSource(
            `http://${BACKEND_URL}:3000/SSE_spielstand`,
        );
        const teams_SSE = new EventSource(
            `http://${BACKEND_URL}:3000/SSE_teams`,
        );
        ...
        spielstand_SSE.onmessage = function (event) {
            ...
            //game.set_aufschlag(data["aufschlag"]);
            if (data["aufschlag"] == 0) {
                game.set_aufschlag(2);
            } else if (data["aufschlag"] == 1) {
                game.set_aufschlag(3);
            } else if (data["aufschlag"] == 2) {
                game.set_aufschlag(0);
            } else if (data["aufschlag"] == 3) {
                game.set_aufschlag(1);
            }
        ...
        teams_SSE.onmessage = function (event) {
            var data = JSON.parse(event.data);
            game.left_team = [data.right_team[1], data.right_team[0]];
            game.right_team = [data.left_team[1], data.left_team[0]];
        };
...
<div class="Container">
    {#if new_game_popup}
        <PopUp bind:new_game_popup bind:selected_spielart={spielart}></PopUp>
    {/if}
    ...
    {:else}
        <div class="menu_container">
            ...
            {#if spielart == 1}
                <button onclick={() => (pos_switch_active = !pos_switch_active)}
                ...
                <div class="switch_view_row_container">
                    <div class="switch_view_column_container">
                        ...
                        <Spielfeld
                            ...
                            aufschlag={game.aufschlag}
                        ></Spielfeld>
                        <div class="switch_button_container">
                            <button
                                class="round-button"
                                onclick={() => update_teams("left")}
                            >
                                <img src="Switch_Button.png" alt="Button" />
                            </button>
                        </div>
        ...
        <div class="punkte_button_container">
            <div class="up_down_container">
                <button onclick={() => req_update_spielstand("right", 1)}
                    >&#8593</button
                >
                <button onclick={() => req_update_spielstand("right", -1)}
                    >&#8595</button
            ...
            <p></p>
            <div class="up_down_container">
                <button onclick={() => req_update_spielstand("left", 1)}
                    >&#8593</button
                >
                <button onclick={() => req_update_spielstand("left", -1)}
                    >&#8595</button
